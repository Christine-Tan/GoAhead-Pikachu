...
public class PaymentSubmitter {
	...
	public ResultMessage submit(PaymentListPO list){
		...
		for(PayeePO payee:list.paymentList){
			builder.InsertInto(PayeeTable.tableName,
					PayeeTable.userID_col,
					PayeeTable.account_col,
					PayeeTable.money_col,
					PayeeTable.type_col,
					PayeeTable.comment_col,
					PayeeTable.paymentList_col).Values(
							payee.getUserID(),
