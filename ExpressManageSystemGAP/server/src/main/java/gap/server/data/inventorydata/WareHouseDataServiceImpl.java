...
public class WareHouseDataServiceImpl implements WareHouseDataService{
	...
	private String sector_id_f = "sector_id",type_f = "type",ins_id_f = "ins_id",
				alarmValue_f = "alarmValue",rows_f = "rows",shelves_f = "shelves",units_f = "units";
...
//		System.out.println("开始了");
		try {
			...
			ResultSet re = sql.Select("*").From(sectorTable).Where(ins_id_f).EQUALS(ins_id).AND(sector_id_f).EQUALS(sector_id).excuteQuery();
			if(re.next()){
		...
		case '0': type = "FLEX";break;
		case '1': type = "CAR";break;
		case '2': type = "TRAIN";break;
		case '3': type = "PLANE";break;
		default: System.out.println("WRONG sector_id !!!!!!!!");
	...
	public ResultMessage delete(String ins_id) throws RemoteException {
		...
		sql.DeleteFrom(sectorItemTable).Where(ins_id).EQUALS(ins_id+"0").excute();
		sql.DeleteFrom(sectorItemTable).Where(ins_id).EQUALS(ins_id+"1").excute();
		sql.DeleteFrom(sectorItemTable).Where(ins_id).EQUALS(ins_id+"2").excute();
		sql.DeleteFrom(sectorItemTable).Where(ins_id).EQUALS(ins_id+"3").excute();
		return null;
	}
