...
public class PaymentGetter {
	...
	public ArrayList<PaymentListPO> getPassedPayment(Calendar start,Calendar end){
	...
	class PaymentListMaker extends ListMaker<PaymentListPO>{
		...
		public PaymentListPO getPO(ResultSet resultSet) {
			...
			try {
				...
				String payer = resultSet.
						getString(PaymentListTable.payer_col);
				Date date = resultSet.
						getDate(PaymentListTable.date_col);
			...
			try{
				...
				switch(type){
					...
					case BUSSINESSCLERK:
						...
						//根据人员ID获得名字
						SQLBuilder builder = new SQLBuilder();
						builder.Select("username").From("user")
