...
public class TransFareDataImpl extends UnicastRemoteObject  implements TransFareDataService{
		...
		ListMaker<TransFarePO> transFareListMaker = new ListMaker<TransFarePO>() {
			...
			public TransFarePO getPO(ResultSet resultSet) {
				...
				try {
					...
					String carID = resultSet.getString(driverID_col);
					double money = resultSet.getDouble(money_col);
					return new TransFarePO(money, orderID, carID);
	...
	public boolean deleteTransFare(List<TransFarePO> transFareList) throws RemoteException {
		...
		for(TransFarePO po: transFareList){
			...
			builder.DeleteFrom(tableName).Where(driverID_col).EQUALS(carID)
