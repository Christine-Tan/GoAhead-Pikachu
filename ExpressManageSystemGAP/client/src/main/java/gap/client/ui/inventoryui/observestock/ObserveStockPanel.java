	...
	public ObserveStockPanel(MainFrame frame) {
		...
		period.confirm.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				List<StockinOrderVO> inlist = StockinOrderController.getRequired(beginDate, endDate, ins_id);
				if(inlist!=null){
				...
				List<StockinOrderVO> outlist = StockinOrderController.getRequired(beginDate, endDate, ins_id);
				stockoutTitle = new TitlePanel("出库");
				if(outlist != null){
					
					stockoutList = new ListPanel(outlist);
					String outTotal  = StockinOrderController.getTotalNum(outlist)+"";
				...
				int total = InventoryController.getOneSectorNum("00110011", ins_id);
