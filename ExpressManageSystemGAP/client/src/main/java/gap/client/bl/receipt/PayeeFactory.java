...
public class PayeeFactory {
	...
	public PayeeFactory(ArrayList<AccountPO> accountList,ArrayList<SalaryPO> salaryList,
	...
	private void addUserPayee(List<UserPO> userList){
		...
		for(UserPO oneUser:userList){
			...
			if(oneUser.getType()==UserType.DELIVERY){
				PercentSalaryPayee percentSalaryPayee = 
						new PercentSalaryPayee(oneUser, itr, controller, accountList);
				payeeList.add(percentSalaryPayee);
			}
			else{
				FixedSalaryPayee fixedSalaryPayee = 
						new FixedSalaryPayee(oneUser, itr, accountList);
	...
	private void addTransFarePayee(List<TransFarePO> transFareList){
		...
		for(TransFarePO fare:transFareList){
			TransFarePayee transFarePayee = new TransFarePayee(fare, accountList);
